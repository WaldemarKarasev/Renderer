...
VulkanBuffer::VulkanBuffer(
  ...
  m_Device_->createBuffer(bufferSize, usageFlags, memoryPropertyFlags, buffer, memory);
}
 
VulkanBuffer::~VulkanBuffer() {
  unmap();
  vkDestroyBuffer(m_Device_->device(), buffer, nullptr);
  vkFreeMemory(m_Device_->device(), memory, nullptr);
}
...
VkResult VulkanBuffer::map(VkDeviceSize size, VkDeviceSize offset) {
  ...
  {
    return vkMapMemory(m_Device_->device(), memory, 0, bufferSize, 0, &mapped);
  }
  */
  return vkMapMemory(m_Device_->device(), memory, offset, size, 0, &mapped);
}
...
void VulkanBuffer::unmap() {
  if (mapped) {
    vkUnmapMemory(m_Device_->device(), memory);
    mapped = nullptr;
  }
}
...
VkResult VulkanBuffer::flush(VkDeviceSize size, VkDeviceSize offset) {
  ...
  return vkFlushMappedMemoryRanges(m_Device_->device(), 1, &mappedRange);
}
...
VkResult VulkanBuffer::invalidate(VkDeviceSize size, VkDeviceSize offset) {
  ...
  mappedRange.offset = offset;
  mappedRange.size = size;
  return vkInvalidateMappedMemoryRanges(m_Device_->device(), 1, &mappedRange);
}
